...
def predict_iris(sep_length, sep_width, pet_length, pet_width):
    ...
    return f"Your input values are: <br>Sepal Length: {sep_length}<br>Sepal Width: {sep_width}<br>Petal Length: {pet_length}<br>Petal Width: {pet_width}<br><span style='color:red;'>Forecast Result is: {iris_type}</span>"
